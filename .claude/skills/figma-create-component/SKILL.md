...
- **Source-of-truth drift.** The Figma cell may bind to library variables whose hex no longer matches the published catalogue (token renamed / removed upstream). Before assuming "Figma is correct," cross-check against `./library-tokens.md`.
...
- **Local-only bindings** — sample 4–6 cells across the matrix and confirm every `boundVariables` id starts with `VariableID:<localId>` (no `VariableID:<sharedKey>/...` consumed-library references). Any component-scoped tokens that aren't in `./library-tokens.md` must be documented in `.claude/skills/figma-components/<Name>/design-token.md` with a resolution chain.
...
- The component is an icon glyph — track it via [`./library-components.md`](./library-components.md) instead.
...
- [`./library-tokens.md`](./library-tokens.md) — published variable collection, text styles, elevation. Loaded in step 4 (and consulted throughout).
- [`./library-components.md`](./library-components.md) — published component / icon inventory. Grep before creating any new component or icon.
