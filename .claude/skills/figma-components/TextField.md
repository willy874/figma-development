...
The complete set of Merak design tokens consumed by `<TextField>`. Names below are **Figma variable paths** in the `merak` collection (see `.claude/skills/figma-operator-guide/design-deken.md`). Bind every Figma paint / stroke / effect to one of these — never to a literal value.
