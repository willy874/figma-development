...
Each MUI color resolves to a documented MUI palette hex. Per-variant paint rules are mechanical and align with `<Button>`'s mapping:
...
Both states require `:hover` / `:active` pseudo-classes. The MUI Figma variants treat them as documented states; the runtime applies them via CSS only. Mirror `<Button>`'s state model:
...
**MUI native IconButton diverges here** — its CSS `:hover` rule applies the 0.04 overlay to **all** variants (including contained) on top of the existing background. The MUI Figma extension follows `<Button>`'s convention of swapping to `palette.<color>.dark` for Contained hover/pressed, which the story's `sx` overrides do not currently emulate (Contained hover keeps the same `main` bg and only raises shadow). When the spec is authored against Figma, treat the Button-style behavior as source-of-truth and file a follow-up to align the story `sx`.
...
3. **Story `sx` drift** — the variant-applying `sx` in `IconButton.stories.tsx` is the runtime stand-in for the MUI `Variant` axis. Any change there (e.g. switching Contained hover from "keep main bg" to `palette.dark`) is a real spec-impacting change — update §4 here and audit `figma.spec.md` §6.
