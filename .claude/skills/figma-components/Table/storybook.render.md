...
4. **Border color** — runtime `rgb(224,224,224)` resolves from MUI's `theme.palette.divider` for light mode. Figma binds cell `border-bottom` to `alias/colors/border-default` (`rgba(0,0,0,0.12)`) for dark-mode parity. Acceptable design divergence, tracked in `figma.spec.md` §7.
