...
| `<PinInput> / FormLabel` TEXT fill (resting) | `fills[0]` (text-fill)        | `merak/alias/colors/text-default` ⇒ `paint.label.enabled` (project convention — see §7 #5) | `render.md §4` (Enabled column)     |
| `<PinInput> / FormHelperText` TEXT fill (resting) | `fills[0]` (text-fill)   | `merak/alias/colors/text-default` ⇒ `paint.helper.enabled` (project convention — see §7 #5) | `render.md §5` (Enabled column)     |
| `<PinInput> / Row / Separator` TEXT fill     | `fills[0]` (text-fill)        | `merak/alias/colors/text-default` ⇒ `paint.separator.text`                    | `render.md §3`                                  |
...
| ---------------------------- | --------------------------------------------- | ----------------------------------------------- | -------------------------------------------------- |
| FormLabel `fills[0]`         | `merak/alias/colors/text-default`             | `merak/seed/danger/main` ⇒ `paint.label.error`  | `merak/alias/colors/text-disabled` ⇒ `paint.label.disabled` |
| FormHelperText `fills[0]`    | `merak/alias/colors/text-default`             | `merak/seed/danger/main` ⇒ `paint.helper.error` | `merak/alias/colors/text-disabled` ⇒ `paint.helper.disabled` |
...
- `merak/seed/danger/main` — `paint.label.error` / `paint.helper.error`. Used as the optional Error-state TEXT fill on FormLabel / FormHelperText (per §4.2.1). Resolves to `#D32F2F` in the light theme.
- `merak/seed/primary/main` — referenced indirectly via the nested `<TextField>` cell's Focused border (`paint.cell.border.focused`). Not bound directly on the wrapper.

### 10.2 Alias (semantic)

- `merak/alias/colors/text-default` — `paint.separator.text` + `paint.label.enabled` + `paint.helper.enabled`. Used as the separator `-` glyph fill **and** the resting FormLabel + FormHelperText TEXT fill (project convention — the local collection has no `text-secondary` token; see §7 #5). Resolves to `rgba(0, 0, 0, 0.87)` in the light theme.
- `merak/alias/colors/text-disabled` — `paint.label.disabled` / `paint.helper.disabled`. Used as the optional Disabled-state TEXT fill on FormLabel / FormHelperText (per §4.2.1). Resolves to `rgba(0, 0, 0, 0.38)` in the light theme.

### 10.3 Component-scoped

None — `<PinInput>` does not mint any component-scoped tokens of its own. Every paint reaches a token in the shared `merak/*` family. See [`design-token.md`](./design-token.md) for the audit (declared-empty file kept so future reviewers can confirm the absence is intentional).
