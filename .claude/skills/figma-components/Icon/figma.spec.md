...
| Figma node `3:2722` variant axes change (e.g. someone adds a `Color` or `State` axis) | `figma.spec.md` §3, then update every host spec's `Icon Source` default and re-run `figma-component-sync` to refresh per-variant JSON |
