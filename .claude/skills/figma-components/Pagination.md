...
Token names below are Figma variable paths (`merak` collection), not CSS variables. See `.claude/skills/figma-operator-guide/design-deken.md`.
...
The complete set of Merak design tokens consumed by `<Pagination>`. Names below are **Figma variable paths** in the `merak` collection (see `.claude/skills/figma-operator-guide/design-deken.md`). Bind every Figma paint / stroke / variable to one of these — never to a literal value.
