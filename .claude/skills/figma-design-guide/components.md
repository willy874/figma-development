# Component Index

Source: [Figma file `KQjP6W9Uw1PN0iipwQHyYn`](https://www.figma.com/design/KQjP6W9Uw1PN0iipwQHyYn/MUI-Library)

Total: **54 components** on 1 page. Before creating anything new, grep this file for the name — if it exists, import via `importComponentByKeyAsync` / `importComponentSetByKeyAsync` and reuse. Never recreate an existing component.

**Variant counts** in parentheses indicate the node is a `COMPONENT_SET` with that many variants; entries without a count are single `COMPONENT` nodes.

---

## Page: `Foundation Components` (id `0:1`) — 54 components

### Component sets & primitives — 14 entries

| Name                     | Node ID    | Notes      |
| ------------------------ | ---------- | ---------- |
| `<Button>`               | `1:4109`   | SET (90)   |
| `<IconButton>`           | `1:4571`   | SET (90)   |
| `<DialogActions>`        | `1:4757`   |            |
| `<DialogContent>`        | `1:4761`   | SET (2)    |
| `<DialogTitle>`          | `1:4768`   |            |
| `<Dialog>`               | `1:4772`   | SET (5)    |
| `<PaginationItem>`       | `1:5098`   | SET (288)  |
| `<Pagination>`           | `1:5675`   | SET (36)   |
| `<TextField>`            | `1:6266`   | SET (60)   |
| `<Checkbox>`             | `1:7228`   | SET (258)  |
| `<CheckboxFormControl>`  | `1:7367`   | SET (276)  |
| `<FormLabel>`            | `1:7696`   | SET (6)    |
| `<DirectionFormControl>` | `1:7709`   | SET (4)    |
| `<Icon>`                 | `3:2722`   | SET (6) — variant wrapper |

### Icon library — 40 entries

All icons are monochrome and intended to be used inside `<IconButton>`, `<Button>`, `<Chip>`, etc.

| Name               | Node ID    | Notes |
| ------------------ | ---------- | ----- |
| ArrowSolid         | `3:2740`   |       |
| ArrowFilled        | `3:2744`   |       |
| User               | `3:2748`   |       |
| Detail             | `3:2754`   |       |
| Close              | `3:2759`   |       |
| Delete             | `3:2765`   |       |
| Lock               | `3:2769`   |       |
| Unlock             | `3:2775`   |       |
| Eye                | `3:2781`   |       |
| EyeClose           | `3:2785`   |       |
| Email              | `3:2789`   |       |
| EmailSend          | `3:2793`   |       |
| Logout             | `3:2797`   |       |
| Loading            | `3:2801`   |       |
| Route              | `3:2805`   |       |
| Audit              | `3:2809`   |       |
| Log                | `3:2813`   |       |
| Menu               | `3:2817`   |       |
| Admin              | `3:2821`   |       |
| Edit               | `3:2825`   |       |
| Verified           | `3:2829`   |       |
| UserChecked        | `3:2833`   |       |
| UserDisabled       | `3:2837`   |       |
| CheckCircleSolid   | `3:2841`   |       |
| CheckCircleFilled  | `3:2845`   |       |
| WarnCircleSolid    | `3:2849`   |       |
| WarnCircleFilled   | `3:2853`   |       |
| DangerCircleSolid  | `3:2857`   |       |
| DangerCircleFilled | `3:2861`   |       |
| More               | `3:2865`   |       |
| Download           | `3:2869`   |       |
| Copy               | `3:2873`   |       |
| Check              | `3:2877`   |       |
| Upload             | `3:2881`   |       |
| Link               | `3:2885`   |       |
| Empty              | `3:2889`   |       |
| i18n               | `3:2896`   |       |
| SelectArrow        | `3:2900`   |       |
| Search             | `3:2904`   |       |
| Add                | `3:2908`   |       |
