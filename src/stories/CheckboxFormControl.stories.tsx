...
// `<CheckboxFormControl>` is the project's name for the composed pattern
// MUI ships as `FormControlLabel + Checkbox`. There is no separate
...
const buildControl = (extras: CheckboxControlExtras = {}) => (
  <Checkbox
...
const meta = {
  title: 'Components/CheckboxFormControl',
  ...
  parameters: {
    docs: {
      description: {
        component:
          'Mirrors the `<CheckboxFormControl>` Figma spec at `.claude/skills/figma-components/CheckboxFormControl/figma.spec.md` — 276 published variants (4 LabelPlacements × 7 Colors × 3 Checked/Indeterminate combos × 3 Sizes, plus a 24-cell Disabled coverage limited to `Color=Default`). The Figma set wraps a `<Checkbox>` instance plus a sibling label; the runtime equivalent is `<FormControlLabel control={<Checkbox/>} label="Label" labelPlacement={...} />`.',
      },
    },
  },
  argTypes: {
          ...
          {VALUE_COMBOS.map(({ label, checked, indeterminate }) => (
            <div key={label} style={placementCell}>
              <FormControlLabel
                ...
                label="Label"
                control={buildControl({ checked, indeterminate })}
              />
            </div>
          ))}
        </Stack>
          ...
          {VALUE_COMBOS.map(({ label, checked, indeterminate }) => (
            <div key={label} style={placementCell}>
              <FormControlLabel
                labelPlacement="end"
                label="Label"
                control={buildControl({ color: mui, checked, indeterminate })}
              />
            </div>
          ))}
        </Stack>
          ...
          {VALUE_COMBOS.map(({ label, checked, indeterminate }) => (
            <div key={label} style={placementCell}>
              <FormControlLabel
                labelPlacement="end"
                label="Label"
                control={buildControl({ size, checked, indeterminate })}
              />
            </div>
          ))}
        </Stack>
...
export const StateMatrix: Story = {
  ...
  render: () => (
    <Stack spacing={1.5}>
      ...
      <span style={{ fontSize: 11, color: '#888' }}>
        {'Hovered / Focused / Pressed are not published in this set. Drop a bare `<Checkbox>` and compose the label manually if those states are needed.'}
      </span>
    </Stack>
  ),
};
